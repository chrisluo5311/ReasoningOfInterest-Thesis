...
\begin{figure}[H]
    ...
    {Image/all_model_comparison2.png}
...
& \textbf{91.0} & \textbf{53.0} & \textbf{48.0} & \textbf{13.3} & \textbf{10.0} \\
\bottomrule
\end{tabular}%
}
...
quiz performance is predictive of the correctness of final answer.
To test this assumption, we analyze the relationship between quiz and exam outcomes across all $480 \times 5 = 2{,}400$
individual runs.

\paragraph{Contingency analysis.}
Table~\ref{tab:quiz-exam-contingency-gsm8k-math} presents the $2 \times 2$
contingency table.  When all quizzes pass, the exam failure rate is
only 1.63\%.  When any quiz fails, it rises to 17.44\%, approximately
\textbf{10.7 times higher}.
This confirms that quiz failures capture meaningful intermediate
reasoning deficiencies rather than acting as noise.
...
Figure~\ref{fig:dose-response} show that the probability of exam failure 
...
\label{fig:dose-response-gsm8k-math}
\end{figure}


The near exponential growth suggests that quiz failures are not
...
Both analyses reveal a consistent positional gradient. Quiz failures
and low-saliency chunks are concentrated toward the end of the
reasoning trace.  This pattern is consistent with error accumulation in
sequential reasoning, where small mistakes in earlier steps compound
and turn into failures in later integration steps.
Since reasoning failures are most concentrated in the final chunk, focusing training on late-stage reasoning steps is likely to produce the largest gains in overall accuracy.
...
Of these 20 items, 18 failed all 5 exam runs with $E_k = 0$, and 2
produced malformed outputs ($E_k = -1$).  However, several degenerate
items achieved high quiz scores despite consistent exam failure.
For instance, \texttt{gsm8k\_1682} obtained perfect quiz scores
($Q_{k,j} = 1$ for all $k, j$) while failing the final exam on every
run. Outcome-only supervision treats these cases the same as problems with incorrect reasoning throughout, even though the intermediate steps are mostly correct and the failure occurs only at the final computation.
...
\begin{table}[H]
\centering
\caption{Saliency variance by correct rate group (GSM8K vs.\ MATH).
...
\end{axis}
\end{tikzpicture}
\caption{Mean saliency variance by correct rate group (GSM8K vs.\ MATH).
...
The data reveals that problems with medium correct rates
(0.4--0.79) show 3--4 times higher saliency variance than
perfectly solved problems.  This confirms that saliency analysis is
most informative at the intermediate difficulty range, where the model
occasionally fails and the saliency distribution reveals which
reasoning chunks are responsible for the inconsistency.  For perfectly
solved problems, saliency is uninformative and for
consistently failed problems, saliency is degenerate.  The most
actionable diagnostic signal lies in the middle, where chunk-level credit assignment helps pinpoint specific reasoning steps that can be directly improved through targeted reinforcement.
...
The aggregate saliency analysis yields several findings with
implications for training and reward design:

\begin{enumerate}[nosep]
\item \textbf{Quiz rewards are predictive, not decorative.}
    Quiz failure increases the probability of exam failure by a factor
    of 10.73$\times$ (Table~\ref{tab:quiz-exam-contingency}), and the
    scaling pattern in Table~\ref{tab:dose-response} confirms
    that quiz performance provides a graded signal rather binary
    noise.  This suggests the quiz rewards capture meaningful information for
    intermediate reasoning quality.

\item \textbf{Late Stage Reasoning Is More Error Prone.}
    Reasoning failures tend to occur in later reasoning chunks. Both quiz failure rates and weak-chunk positions show a clear positional trend, with the final chunk being the most error-prone (Figure~\ref{fig:positional-analysis}). This pattern suggests that errors accumulate over the course of reasoning and motivates our targeted reinforcement strategies.

\item \textbf{Non-uniform saliency signals fragile reasoning.}
    Single-dominant problems have the lowest correct rate (0.813)
    among all patterns, and medium-difficulty problems exhibit
    3--4 times higher saliency variance
    (Table~\ref{tab:correct-rate-bins}), proving that saliency
    structure is correlated with reasoning reliability.

\item \textbf{Saliency is most diagnostic in the intermediate
    difficulty range.}  For problems the model consistently solves
    (64.6\% perfect-score uniform), saliency provides no actionable
    information. For consistently failed problems, saliency is
    degenerate.  The most valuable diagnostic signal lies in the
    \textasciitilde{}35\% of problems where the model presents
    partial success, making saliency guided targeted reinforcement
    (Section~\ref{sec:targeting-weak-chunk}) most impactful for this subset.

\item \textbf{Quiz--exam decoupling reveals hidden credit assignment
    failures.}  A subset of degenerate items achieves high quiz scores
    but fails every exam, demonstrating that outcome-only supervision
    cannot distinguish fundamentally flawed reasoning from correct
    intermediate reasoning with a faulty final step.
\end{enumerate}


Saliency analysis makes it possible to identify which reasoning steps contribute most to the final answer and which ones are weakest. By directing training toward these fragile segments, the model can improve its reasoning without any human-annotated intermediate steps. This supports the central motivation of our approach, that automated quiz and exam feedback is sufficient to diagnose and correct reasoning deficiencies, leading to more robust reasoning behavior.





\subsection{Targeted Improvements on Weak Reasoning Chunks}
